...
class HomeController extends Controller {
    ...
    {        
        ...
        $conn = DB::connection('mysql');
        $sql = "SELECT a.id,a.product_name, b.image, c.sortname, co.company_name FROM catalogues a
                JOIN catalogue_images b on a.id = b.catalogue_id
                JOIN users u on a.user_id = u.id
                JOIN company co on u.id = co.user_id
                JOIN countries c on u.country_id = c.id
                WHERE flag = 0 AND a.iklan = 0
                GROUP BY a.id";
        $dataAll = $conn->select($sql);
        $All = $dataAll;
        ...
        return view('advertisement', compact('products', 'categories', 'flag', 'country', 'countriesRequest','inquiry','Buy','Sell','All'));    
    ...
    public function productCatalogue()
    {
        ...
        $productlow = DB::table('catalogues')
            ->join('catalogue_images','catalogues.id','=','catalogue_images.catalogue_id')
            ->join('catagories','catalogues.product_category','=','catagories.id')
            ->join('users', 'users.id', '=', 'catalogues.user_id')
            ->join('company', 'company.user_id', '=', 'users.id')
        ...
        return view('productCatalogue', compact('productlow'));
    }
